...
function buildVolumeMounts(
  ...
  // Copy .mcp.json into group's .claude directory so Agent SDK can find it
  const mcpConfigSrc = path.join(projectRoot, '.mcp.json');
  if (fs.existsSync(mcpConfigSrc)) {
    const mcpConfigDst = path.join(groupSessionsDir, '.mcp.json');
    fs.copyFileSync(mcpConfigSrc, mcpConfigDst);
  }
